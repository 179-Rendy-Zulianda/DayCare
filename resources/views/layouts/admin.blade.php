...
<head>
    ...
    <title>@yield('title', 'Admin - Candice Kids')</title>
...
<body class="bg-gradient-to-br from-sky-200 via-purple-100 to-pink-100 font-sans min-h-screen">
    ...
    <div class="flex min-h-screen">
        ...
        <aside class="hidden lg:block bg-white/80 backdrop-blur-sm w-64 p-6 shadow-xl">
            ...
            <ul class="space-y-3">
                <li class="{{ request()->routeIs('admin.dashboard') ? 'bg-white shadow font-bold text-gray-900' : '' }} rounded-xl px-4 py-3 flex items-center gap-3">
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 w-full">
                        <span class="text-xl">🛠️</span>
                        <span>Dashboard Admin</span>
                    </a>
                </li>
                <li class="{{ request()->routeIs('admin.user') ? 'bg-white shadow font-bold text-gray-900' : '' }} rounded-xl px-4 py-3 flex items-center gap-3">
                    <a href="{{ route('admin.user') }}" class="flex items-center gap-3 w-full">
                        <span class="text-xl">🧑‍💼</span>
                        <span>Data User</span>
                    </a>
                </li>
                <li class="{{ request()->routeIs('admin.anak') ? 'bg-white shadow font-bold text-gray-900' : '' }} rounded-xl px-4 py-3 flex items-center gap-3">
                    <a href="{{ route('admin.anak') }}" class="flex items-center gap-3 w-full">
                        <span class="text-xl">🧒</span>
                        <span>Data Anak</span>
                    </a>
                </li>
                <!-- <li class="{{ request()->routeIs('admin.tagihan') ? 'bg-white shadow font-bold text-gray-900' : '' }} rounded-xl px-4 py-3 flex items-center gap-3">
                    <a href="{{ route('admin.tagihan') }}" class="flex items-center gap-3 w-full">
                        <span class="text-xl">💳</span>
                        <span>Data Tagihan</span>
                    </a>
                </li> -->
                <li class="{{ request()->routeIs('admin.aktivitas') ? 'bg-white shadow font-bold text-gray-900' : '' }} rounded-xl px-4 py-3 flex items-center gap-3">
                    <a href="{{ route('admin.aktivitas.index') }}" class="flex items-center gap-3 w-full">
                        <span class="text-xl">📝</span>
                        <span>Data Aktivitas</span>
                    </a>
                </li>
                <li class="{{ request()->routeIs('admin.notifikasi') ? 'bg-white shadow font-bold text-gray-900' : '' }} rounded-xl px-4 py-3 flex items-center gap-3">
