...
<div class="bg-white rounded-2xl shadow-xl p-6 max-w-lg mx-auto mt-8 mb-8">
    <h2 class="text-xl font-bold mb-4 text-orange-700">Tambah Aktivitas Anak</h2>
    <form action="{{ route('admin.aktivitas.store') }}" method="POST" class="space-y-4">
        @csrf
        <div>
            ...
            <select name="child_id" class="w-full border rounded px-3 py-2" required>
                ...
                <option value="{{ $child->id }}">{{ $child->nama_anak }}</option>
                @endforeach
            </select>
        </div>
        <div>
        ...
        <div>
            ...
            <input type="time" name="jam" class="w-full border rounded px-3 py-2">
        </div>
        <div>
        ...
        <div class="flex justify-end gap-2">
            ...
            <button type="submit" class="px-4 py-2 rounded bg-gradient-to-r from-orange-400 to-pink-400 text-orange-900 font-bold shadow">Simpan</button>
        </div>
    </form>
</div>
@endsection 